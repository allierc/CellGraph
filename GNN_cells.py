...
class InteractionParticles(pyg.nn.MessagePassing):
    ...
    def __init__(self, model_config, device):   # in_feats=9, out_feats=2, num_layers=2, hidden=16):
        ...
        if (self.msg==1) | (self.msg==2):
            self.lin_edge = pyg.nn.MLP(in_channels=9, hidden_channels=self.hidden_size0, out_channels=self.output_size0, num_layers=self.nlayers0, act='relu', device=self.device, dtype=torch.float64)
    ...
    def message(self, x_i, x_j,edge_attr):
        ...
        if self.msg==0:
            ...
            in_features = torch.cat((delta_pos, d, x_i_vx, x_i_vy, x_j_vx, x_j_vy, erk_i, erk_j), dim=-1)
        elif self.msg==3:
            cell_id_i = x_i[:, 1].detach().cpu().numpy()
            cell_id_j = x_j[:, 1].detach().cpu().numpy()
            embedding = torch.cat((self.a[self.data_id,cell_id_i],self.a[self.data_id,cell_id_i]),axis=-1)
            in_features = torch.cat((delta_pos, d, x_i_vx, x_i_vy, x_j_vx, x_j_vy, erk_i, erk_j, embedding), dim=-1)
    ...
    for epoch in range(60):
        ...
        if training_mode == 't+1':
            for N in range(1, np.sum(model_config['frame_end']) * data_augmentation_loop // batch_size):
                ...
                loss = criteria(pred*mask_batch, y_batch*mask_batch) * 3
                loss.backward()
                optimizer.step()
    ...
    def __init__(self, model_config, device):
        ...
        if self.embedding > 0:
            self.layer = torch.nn.ModuleList(
                [InteractionNetworkEmb(layers=3, embedding=self.embedding, h=self.upgrade_type, device=self.device) for _ in range(self.nlayers)])
...
def train_model_ResNet(model_config=None, trackmate_list=None, nstd=None, nmean=None, n_tracks_list=None):
    ...
    for epoch in range(1000):
        ...
        for data_id in range(len(model_config['dataset'])):
            ...
            print(f"data_id: {data_id} Epoch: {epoch} Loss: {np.round(np.mean(loss_list), 4)}")

            if (np.mean(loss_list) < best_loss) & (data_id==0)  :
                best_loss = np.mean(loss_list)
                torch.save({'model_state_dict': model.state_dict(), 'optimizer_state_dict': optimizer.state_dict()},
            ...
            if (epoch%100==0):
                torch.save({'model_state_dict': model.state_dict(), 'optimizer_state_dict': optimizer.state_dict()},
                           os.path.join(log_dir, 'models', f'model_{epoch}.pt'))
