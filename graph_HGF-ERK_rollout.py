...
class InteractionParticlesRollout(pyg.nn.MessagePassing):
    ...
    def __init__(self, model_config, device):   # in_feats=9, out_feats=2, num_layers=2, hidden=16):
        ...
        self.t = nn.Parameter(torch.tensor(np.ones((3,241, 1)), device='cuda:0', requires_grad=True))
        ...
        self.b = nn.Parameter(torch.tensor(np.ones((3,int(self.n_tracks+1),3)), device='cuda:0', requires_grad=True))
